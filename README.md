...
При первом запуске демо-данные создаются автоматически:
...
**Отключить автоматическое создание данных:**

Удалите файл `mu-plugins/doctors-demo-data.php`:

```bash
rm doctors-plugin/mu-plugins/doctors-demo-data.php
docker-compose exec wordpress rm -rf /var/www/html/wp-content/mu-plugins/doctors-demo-data.php
```
